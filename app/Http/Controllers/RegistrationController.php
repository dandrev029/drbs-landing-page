...
class RegistrationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('registration.success', ['registration' => $registration->registration_number])
                ->with('success', 'Your registration has been submitted successfully! Your registration number is: ' . $registration->registration_number . '. We will contact you within 24 hours.');

        } catch (\Exception $e) {
            \Log::error('Registration submission error: ' . $e->getMessage());
            return back()
                ->withInput()
                ->with('error', 'An error occurred while submitting your registration. Please try again or contact us directly.');
        }
    }
    ...
    public function track(Request $request)
    {
        ...
        if ($validator->fails()) {
            return back()
                ->withErrors($validator)
                ->with('error', 'Please enter a registration number or email address.');
        }
        ...
        if (filter_var($input, FILTER_VALIDATE_EMAIL)) {
            // Search by email
            $registration = Registration::where('email', $input)->latest()->first();
        } else {
            // Search by registration number
            $registration = Registration::where('registration_number', $input)->first();
        }

        if (!$registration) {
            return back()
                ->withInput()
                ->with('error', 'No registration found with the provided information. Please check and try again.');
        }

        return redirect()->route('registration.show', ['registration' => $registration->registration_number]);
    }
    ...
    public static function getStatistics()
    {
        return [
            ...
            'this_month' => Registration::whereMonth('created_at', now()->month)->count(),
